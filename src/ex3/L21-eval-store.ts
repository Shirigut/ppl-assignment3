...
import { Result, bind, safe2, mapResult, makeFailure, makeOk, isOk, isFailure , either} from "../shared/result";
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses: number[] = map((x : Value) => extendStore(theStore, x).vals.length-1, args);
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        const addresses : number[] = map((v : Value) => extendStore(theStore, v).vals.length-1, vals);
...
const evalSet =(exp: SetExp, env: Env): Result<void> => {
    const index: Result<number> = applyEnv(env, exp.var.var)
    if (isFailure(index)) makeFailure("index does not exist")
    const value: Result <Value> = applicativeEval(exp.val, env)
    if(isFailure(value)) makeFailure("error");
    if(isOk(index) && isOk(value)) return makeOk(setStore(theStore, index.value, value.value));
